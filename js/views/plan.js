...
import { planFor, BANDS, fmt, fmtWeight, fmtSigned, relativeDate, snapReps, REP_SCHEMES, SET_COLUMNS } from '../metrics.js';
...
export function renderPlan(ctx) {
  ...
  root.append(el('header', { class: 'view-head' }, [
    ...
    el('p', { class: 'view-sub', text: 'One prescription per lift, sized to beat your last session by your weekly target.' }),
  ]));
  ...
  if (!trained.length) {
    root.append(el('div', { class: 'empty' }, [
      ...
      el('button', { type: 'button', class: 'btn btn-primary', onclick: () => ctx.goTo('log') }, ['Log your first set']),
    ]));
    return root;
  }
  ...
  root.append(details('How the planner decides', [
...
function card(stats, ctx, settings) {
  ...
  const head = el('button', {
    ...
    onclick: () => { openId = isOpen ? null : id; ctx.refresh(); },
  }, [
    el('div', { class: 'card-head-main' }, [
      el('h2', { class: 'card-title', text: stats.exercise.name }),
      el('p', { class: 'card-meta', text: stats.lastDate
  ...
  body.append(el('div', { class: 'presc' }, [
    el('div', { class: isOpen ? 'presc-hero' : 'presc-line' }, [
      el('span', { class: 'presc-scheme', text: `${plan.sets} × ${plan.reps}` }),
  ...
  body.append(el('p', { class: 'presc-explain', text:
  ...
  if (plan.band.key === 'beaten' && Number.isFinite(stats.bestAdj)) {
    body.append(el('button', {
      ...
      el('span', { class: 'hint-value', text: `Your target comes from last session (${fmt(stats.lastAdj, 1)}), which is under your best of ${fmt(stats.bestAdj, 1)}. Aim past the best instead →` }),
    ]));
  }
  ...
  return el('article', { class: `card${isOpen ? ' is-open' : ''}` }, [head, body]);
}
...
function detail(stats, plan, ctx, settings) {
  ...
  const targetRow = el('div', { class: 'target-row' }, [
    el('div', { class: 'target-facts' }, [
      factLine('Target adj e1RM', `${fmt(plan.target, 1)} kg`, plan.usingManualTarget ? 'your override' : `last ${fmt(stats.lastAdj, 1)} + ${(stats.gainPerWeek * 100).toFixed(2)}%/wk`),
      factLine('Current best', `${fmt(stats.bestAdj, 1)} kg`, stats.bestAdj > stats.lastAdj + 1e-9 ? 'beat this to set a PR' : 'set last session'),
    ]),
    stepper({
      label: 'Override target (kg)', value: o.target ?? '', step: 0.5, min: 0, max: 999, dp: 1, id: `tgt-${id}`,
  ...
  if (plan.gentlest && plan.gentlest.score < plan.score - 1e-9) {
    wrap.append(el('button', {
      ...
      el('span', { class: 'hint-value', text: `${plan.sets} × ${plan.gentlest.reps} @ ${fmtWeight(plan.gentlest.weight)} kg — scores ${fmt(plan.gentlest.score, 1)}` }),
    ]));
  }
  ...
  const seg = segmented({
    ...
    options: [{ value: 'weights', label: 'Weight' }, { value: 'scores', label: 'Scores' }, { value: 'table', label: 'Table' }],
  ...
  if (o.reps !== null || o.sets !== null || o.target !== null) {
    wrap.append(el('button', {
      ...
      onclick: () => { overrides.set(id, { target: null, reps: null, sets: null }); ctx.refresh(); toast('Back to automatic'); },
    }, ['Reset to automatic']));
  }
  return wrap;
}
...
function gridFor(mode, stats, plan, ctx, settings) {
  ...
  const table = el('table', { class: 'grid', 'aria-label': scores ? 'Adjusted e1RM each option scores' : 'Weight to lift for each reps and sets option' });
  const thead = el('thead', {}, [el('tr', {}, [
    el('th', { class: 'grid-corner', scope: 'col' }, [el('span', { text: 'reps' }), el('small', { text: 'sets →' })]),
    ...
    for (const [ci, sets] of SET_COLUMNS.entries()) {
      ...
      tr.append(el('td', { class: `cell is-${cell.band.key}${isPick ? ' is-pick' : ''}` }, [
        el('button', {
          ...
          onclick: () => { o.reps = reps; o.sets = sets; ctx.refresh(); },
        }, [
...
function tableView(stats, plan) {
  ...
  const table = el('table', { class: 'data-table' }, [
    ...
    el('thead', {}, [el('tr', {}, [
      el('th', { scope: 'col', text: 'Reps' }), el('th', { scope: 'col', text: 'Weight' }),
      el('th', { scope: 'col', text: 'Scores' }), el('th', { scope: 'col', text: 'Volume' }),
