...
function viewSwitch(ctx) {
  return el("div", { class: "view-switch" }, [
    segmented({
      ...
      value: viewMode(),
      options: [
        { value: "lifts", label: "Your lifts" },
        { value: "body", label: "Everything" },
      ],
      onChange: (v) => {
        ui.set(VIEW, v);
...
function listView(ctx) {
  ...
  root.append(
    el("header", { class: "view-head" }, [
      ...
      el("p", {
        class: "view-sub",
        text: "One number per lift, and which way it is going. Tap a lift for its chart and history.",
      }),
    ]),
  );
  ...
  if (!trained.length) {
    root.append(
      emptyState(
        "Nothing to show yet",
        "Log a couple of sessions and your trends appear here.",
        "Go to Log",
        () => ctx.goTo("log"),
      ),
    );
  ...
  for (const s of stats)
    for (const e of s.entries)
      if (e.day >= dayNumber(ctx.today) - 7) days7.add(e.date);
  ...
  const reps7 = stats.reduce(
    (n, s) => n + (isBodyweight(s.exercise) ? s.work7 : 0),
    0,
  );
  const hard7 = stats.reduce((n, s) => n + s.hardSets7, 0);
    ...
    el("div", { class: "kpi-row" }, [
      ...
      vol7 > 0
        ? statTile({
            ...
            value: fmtCompact(vol7),
            unit: "kg",
          })
        : statTile({ label: "Reps, last 7 days", value: fmtCompact(reps7) }),
      statTile({ label: "Sessions, last 7 days", value: String(days7.size) }),
      statTile({
        label: "Lifting heavier",
        value: `${rising}/${trained.length}`,
      }),
      // Sets, and then the ones that actually count: in a rep range that builds
      // size, and near enough to failure to have done it.
      statTile({ label: "Hard sets, last 7 days", value: fmt(hard7, 1) }),
    ]),
  );
  ...
  if (listMode() === "table") {
    ...
    root.append(
      el("p", {
        class: "field-hint",
        text: "Swipe the table sideways for trend, projections and weekly sets.",
      }),
    );
  } else {
  ...
  root.append(
    details("What these numbers mean", [
      el("p", {
        text: "Every lift is boiled down to one number — an estimate of the most you could lift once, adjusted for how many sets you did. It lets 8 reps at 60 kg and 5 reps at 70 kg be compared honestly, so the line goes up when you get stronger rather than when you simply do more reps.",
      }),
      el("p", {
        text: "The trend is that number fitted against time over your recent sessions, in kilos per week. Two sessions is not a trend — give it a few.",
      }),
      el("p", {
        text: "Weekly sets is a plain count of working sets. Roughly 10–20 hard sets per muscle per week is the usual recommendation, spread across every lift that trains it.",
      }),
      disclose(
        "Show the formulas",
        [
          el("p", {
            text: "e1RM (Epley) = weight × (1 + reps/30). It puts 8 reps at 60 kg and 5 reps at 70 kg on the same scale. Adjusted e1RM multiplies that by (1 + k × ln(sets)) — extra sets earn credit with diminishing returns: +3.5% for 2 sets, +5.5% for 3, +8.0% for 5 at the default k of 0.05.",
          }),
          el("p", {
            text: "Volume (weight × reps × sets) is tracked separately because it measures a different thing. Weekly sets is a plain count of working sets, because sets per week is the unit training is actually prescribed in — commonly 10–20 per muscle per week, spread across every lift that trains it.",
          }),
          el("p", {
            text: "Trend is a least-squares fit of adjusted e1RM against date over the lookback window. Projections extend that straight line; real progress decelerates, so treat +12 weeks as an optimistic ceiling rather than a forecast.",
          }),
        ],
...
function bodyView(ctx) {
  ...
  root.append(
    el("header", { class: "view-head" }, [
      ...
      el("p", {
        class: "view-sub",
        text: "Your whole log at once: how much work you are doing, how often, where it is going, and what you have beaten.",
      }),
    ]),
  );
  root.append(viewSwitch(ctx));
  ...
  if (!trained.length) {
    root.append(
      emptyState(
        "Nothing to show yet",
        "Log a few sessions and the whole picture appears here.",
        "Go to Log",
        () => ctx.goTo("log"),
      ),
    );
  ...
  let tonDrawn = false;
  const paintTons = () => {
    const chart = tonnageChart(tons, {
      width: tonHost.clientWidth || 340,
      height: 150,
    });
    if (!chart) {
      tonHost.remove();
    ...
    tonHost.replaceChildren(
      el("h2", { class: "section-title", text: "Work" }),
      chart,
    );
    tonDrawn = true;
  };
    ...
    root.append(
      el("div", { class: "chart-card" }, [
        el("div", { class: "section-head" }, [
          ...
          el("span", {
            class: "rail-sub",
            text: `${trainedDays} of the last ${days.length} days`,
          }),
        ]),
        grid,
      ]),
  ...
  if (prs.length) {
    ...
    for (const p of shown) {
      list.append(
        el("li", { class: "pr-event" }, [
          el("span", {
            class: "pr-when",
            text: relativeDate(p.date, ctx.today),
          }),
          el("span", { class: "pr-lift" }, [
            accentDot(p.exerciseId),
            el("span", { text: p.name }),
          ]),
          el("span", {
            class: "pr-set",
            text: describeSet(
              store.getExercise(p.exerciseId),
              p.sets,
              p.reps,
              p.weight,
            ),
          ...
          el("span", {
            class: "pr-gain",
            text: p.delta !== null ? `+${fmt(p.delta, 1)}` : "first",
          }),
        ]),
      );
    }
    root.append(
      el("div", { class: "chart-card" }, [
        ...
        prs.length > shown.length
          ? el("p", {
              class: "chart-cap",
              text: `The ${shown.length} most recent. Older ones are in each lift\u2019s own history.`,
            })
          : null,
      ]),
    );
  ...
  root.append(
    details("What this screen is and is not", [
      el("p", {
        text: "Work is every lift\u2019s volume added together \u2014 weight times reps times sets. It answers whether a week was heavy or light against your own usual, not against anybody else\u2019s.",
      }),
      el("p", {
        text: "Turning up is a plain count of working sets per day. There is no streak here and nothing to break: a rest day is drawn as a rest day, because rest is part of training rather than a failure to train.",
      }),
      el("p", {
        text: "Where the work went is per lift, and only per lift. Balancing push against pull, or upper against lower, would need to know which muscles each lift trains, and the app does not \u2014 so it does not pretend to.",
      }),
    ]),
  );
  return root;
...
function exerciseRow(st, ctx) {
  const trendGood =
    st.trendPerWeek == null || Math.abs(st.trendPerWeek) < FLAT
      ? null
      : st.trendPerWeek > 0;
  const spark = sparkline(st.sessions.map((s) => s.best.adj));
  const stale = st.lastDate
    ? dayNumber(ctx.today) - dayNumber(st.lastDate)
    : null;
    ...
    [
      el(
        ...
        [
          ...
          st.entryCount
            ? el("div", { class: "row-open-figs" }, [
                el("span", { class: "row-adj-big" }, [
                  fmt(st.lastAdj, 1),
                  el("small", {
                    text: isBodyweight(st.exercise) ? " score" : " kg",
                  }),
                ]),
                st.trendPerWeek != null
                  ? el("span", {
                      class: `row-trend${st.trendReliable && trendGood ? " is-good" : st.trendReliable && trendGood === false ? " is-bad" : " is-muted"}`,
                      text:
                        Math.abs(st.trendPerWeek) < FLAT
                          ? "flat"
                  ...
                  : el("span", {
                      class: "row-trend is-muted",
                      text: "needs 2 sessions",
                    }),
              ])
            : null,
...
function detailView(st, ctx) {
    ...
    el("div", { class: "detail-bar" }, [
      ...
      el(
        "button",
        {
          type: "button",
          class: "link-btn",
          onclick: () => ctx.goTo("plan", { openExercise: st.exercise.id }),
        },
        ["Plan next →"],
      ),
    ]),
  ...
  if (!st.entryCount) {
    root.append(
      emptyState(
        "No history",
        "Log a session for this lift to see it charted.",
        "Log a set",
        () => ctx.goTo("log", { exerciseId: st.exercise.id }),
      ),
    );
    return root;
  }
  ...
  root.append(
    el("div", { class: "hero" }, [
      ...
      el("span", { class: "hero-value" }, [
        heroNum,
        el("small", { text: isBodyweight(st.exercise) ? " score" : " kg" }),
      ]),
      el("span", {
        class: `hero-delta${trendClass(st)}`,
        text: heroTrendText(st),
      }),
    ]),
  );
  ...
  const spanDays = st.sessions.length
    ? st.sessions[st.sessions.length - 1].day - st.sessions[0].day
    : 0;
  const RANGES = [
  ...
  const paintLane = () => {
    const lane = readinessLane(st, settings, {
      width: laneHost.clientWidth || 340,
      height: 104,
      todayIso: ctx.today,
    });
    if (!lane) {
  ...
  root.append(
    el("div", { class: "kpi-row kpi-row-2" }, [
      statTile({
        label: "Best ever",
        value: fmt(st.bestAdj, 1),
        unit: isBodyweight(st.exercise) ? "" : "kg",
        delta: st.prCount
          ? `${st.prCount} PR${st.prCount === 1 ? "" : "s"} so far`
          : null,
      }),
      statTile({
        label: "Next target",
        value: fmt(st.nextTarget, 1),
        unit: isBodyweight(st.exercise) ? "" : "kg",
        delta: targetDelta(st),
        deltaLabel: targetDeltaLabel(st),
      }),
    ]),
  );
  ...
  root.append(
    disclose(
      ...
      [
        el("div", { class: "kpi-row kpi-row-2" }, [
          statTile({
            label: "Projected +4 wks",
            value: st.proj4 != null ? fmt(st.proj4, 1) : "—",
            unit: st.proj4 != null && !isBodyweight(st.exercise) ? "kg" : "",
            delta: st.proj4 == null ? "needs 3 sessions over 2 weeks" : null,
          }),
          statTile({
            label: "Projected +12 wks",
            value: st.proj12 != null ? fmt(st.proj12, 1) : "—",
        ...
        el("p", {
          text:
            "Both numbers extend today’s straight-line trend. Real progress " +
  ...
  const paintVol = () => {
    ...
    volHost.replaceChildren(
      setsMeter(st),
      weeklySetsChart(st, { width, height: 140, todayIso: ctx.today }),
    );
  };
  root.append(volHost);
  ...
  root.append(
    el("div", { class: "kpi-row kpi-row-2" }, [
      isBodyweight(st.exercise)
        ? statTile({
            label: "Reps, last 7 days",
            value: fmtCompact(
              st.entries
                .filter((e) => e.day >= dayNumber(ctx.today) - 7)
                .reduce(
                  (n, e) =>
                    n + e.reps * (Number(e.sets) > 0 ? Number(e.sets) : 1),
                  0,
                ),
            ),
          })
        : statTile({
            label: "Volume, last 7 days",
            value: fmtCompact(st.volume7),
            unit: "kg",
          }),
      statTile({
        label: "Last session",
        value: `${st.lastSets} × ${st.lastReps}`,
        deltaLabel: "",
        delta: isBodyweight(st.exercise)
          ? "reps"
          : `@ ${fmtWeight(st.sessions[st.sessions.length - 1].best.weight)} kg`,
      }),
    ]),
  );

  // The two things a lift can be doing: getting stronger, and getting enough
  // of the work that grows it. They are not the same achievement.
  root.append(
    el("div", { class: "kpi-row kpi-row-2" }, [
      statTile({
        ...
        deltaLabel:
          st.trendPerWeek != null && !st.trendReliable ? "provisional" : "",
      }),
      statTile({
        label: "Hard sets, last 7 days",
        value: fmt(st.hardSets7, 1),
        unit: st.setsPerWeekTarget ? `of ${st.setsPerWeekTarget}` : "",
        good: st.hardSetStatus ? st.hardSetStatus === "on target" : undefined,
        deltaLabel: st.hardSetStatus || "",
      }),
    ]),
  );

  if (st.sets7 > 0 && st.hardSets7 < st.sets7 - 0.05) {
    root.append(
      el("p", {
        class: "card-note",
        text:
          `${st.sets7} sets logged this week, worth ${fmt(st.hardSets7, 1)} hard ones. ` +
          `Sets outside ${ZONES.hypertrophy.minReps}–${ZONES.hypertrophy.maxReps} reps, or taken well short of ` +
          "failure, count for less — they are still training, just not the kind that builds size.",
      }),
    );
  }
...
function sessionTable(st, settings) {
  ...
  return el("div", { class: "table-scroll" }, [
    el("table", { class: "data-table" }, [
      el("caption", {
        text: "Best set of each session. Adj e1RM is what the chart plots.",
      }),
      el("thead", {}, [
        el("tr", {}, [
          el("th", { scope: "col", text: "Date" }),
          el("th", { scope: "col", text: "Best set" }),
          el("th", { scope: "col", text: "Adj" }),
          el("th", { scope: "col", text: "Sets" }),
      ...
      el(
        ...
        rows.map((s) =>
          el("tr", { class: s.best.isPR ? "is-pr" : "" }, [
            ...
            el("td", {}, [
              fmt(s.best.adj, 1),
              s.best.isPR ? prBadge({ compact: true }) : null,
            ]),
...
function targetDelta(st) {
  const r = st.readiness;
  if (!r || !r.enabled || st.lastAdj == null)
    return `+${(st.gainPerWeek * 100).toFixed(2)}%`;
...
function targetDeltaLabel(st) {
  ...
  switch (r.phase.key) {
    case "recovering":
      return "on last, held back for fatigue";
    case "detrained":
      return "on last, after the layoff";
    default:
      return `on last, ${r.days === 1 ? "a day" : `${r.days} days`} ago`;
  }
}
...
function dashboardTable(stats) {
  return el("div", { class: "table-scroll" }, [
    el("table", { class: "data-table data-table-wide" }, [
      el("caption", {
        text: "The Dashboard sheet, one row per lift. * = provisional fit, fewer than 3 sessions or under 2 weeks. Flat target is the spreadsheet’s — last session plus the weekly gain; Next target is that number after fatigue and detraining.",
      }),
      el("thead", {}, [
        el(
          "tr",
        ...
        stats.map((s) =>
          el("tr", {}, [
            ...
            el("td", {
              text:
                s.trendPerWeek != null
                  ? fmtSigned(s.trendPerWeek, 2) + (s.trendReliable ? "" : "*")
                  : "—",
            }),
            el("td", { text: s.proj4 != null ? fmt(s.proj4, 1) : "—" }),
            el("td", { text: s.proj12 != null ? fmt(s.proj12, 1) : "—" }),
            el("td", {
              text: s.baseTarget != null ? fmt(s.baseTarget, 1) : "—",
            }),
            el("td", {
              text: s.nextTarget != null ? fmt(s.nextTarget, 1) : "—",
            }),
            el("td", { text: fmtCompact(s.work7) }),
            el("td", { text: String(s.sets7) }),
            el("td", { text: fmt(s.hardSets7, 1) }),
            el("td", {
              text: s.setsPerWeekTarget ? String(s.setsPerWeekTarget) : "—",
            }),
            el("td", {}, [
              s.setStatus
                ? el(
                    "span",
                    {
                      class: `status-chip is-${s.setStatus.replace(" ", "-")}`,
                    },
                    [
                      el("span", {
                        class: "status-glyph",
...
function heroTrendText(st) {
  ...
  if (Math.abs(st.trendPerWeek) < FLAT)
    return "holding steady over your recent sessions";
  ...
  return (
    `${dir} about ${Math.abs(st.trendPerWeek).toFixed(2)}${unit} a week` +
