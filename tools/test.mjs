...
const fixture = JSON.parse(
  readFileSync(join(here, "xlsx-fixture.json"), "utf8"),
);
...
function close(name, actual, expected, tol = 0.001) {
  ...
  ok(
    name,
    Number.isFinite(a) && Math.abs(a - e) <= tol,
    `got ${a}, sheet says ${e}`,
  );
}
...
close(
  "ladder: an odd stack, base 11.5 step 2.3",
  M.ceilToStep(60, 2.3, 11.5),
  62.1,
);
close(
  "ladder: float dust cannot skip a rung",
  M.ceilToStep(0.1 + 0.2 + 27.2, 2.5, 20),
  27.5,
);
...
// weightForTarget lands on the ladder, and never suggests less than the bar.
{
  ...
  ok(
    "weightForTarget clears the target",
    w * M.repFactor(5, "epley") * M.setBonus(3, 0.05) >= 120 - 1e-9,
  );
  ok(
    "weightForTarget never goes under the bar",
    M.weightForTarget(1, 12, 5, st, 2.5, 20) === 20,
  );
}
...
{
  ok(
    "barely above the bar needs no ramp at all",
    M.warmupPlan(25, 2.5, 20, "weight").length === 0,
  );
  ok(
    "the bar itself needs nothing to ramp through",
    M.warmupPlan(20, 2.5, 20, "weight").length === 0,
  );

  const heavy = M.warmupPlan(100, 2.5, 20, "weight");
  ok(
    "a heavy lift gets the full climb",
    heavy.length === 5,
    JSON.stringify(heavy),
  );
  ok("the bar leads it off", heavy[0].weight === 20);
  ok(
    "every step is strictly heavier than the last",
    heavy.every((s, i) => i === 0 || s.weight > heavy[i - 1].weight),
  );
  ok(
    "every step stays under the work weight",
    heavy.every((s) => s.weight < 100),
  );
  ok(
    "the reps taper 10, 5, 3, 1 after the bar",
    heavy
      .slice(1)
      .map((s) => s.reps)
      .join(",") === "10,5,3,1",
  ...
  ok(
    "every step lands exactly on this lift's own ladder",
    odd.every(
      (s) =>
        Math.abs(
          (s.weight - 11.5) / 2.3 - Math.round((s.weight - 11.5) / 2.3),
        ) < 1e-6,
    ),
    JSON.stringify(odd),
  );
  ...
  ok(
    "a reps-kind lift gets no ramp, whatever the work weight",
    M.warmupPlan(100, 2.5, 20, "bodyweight").length === 0,
  );
  ok(
    "nor does an invalid work weight",
    M.warmupPlan(NaN, 2.5, 20, "weight").length === 0 &&
      M.warmupPlan(0, 2.5, 20, "weight").length === 0,
  );

  ok(
    "no bar step when there is nothing to load from (base 0)",
  ...
  ok(
    "a coarse step never repeats a weight",
    coarse.every((s, i) => i === 0 || s.weight > coarse[i - 1].weight),
    JSON.stringify(coarse),
  );
  ok(
    "and never reaches the work weight",
    coarse.every((s) => s.weight < 100),
  );
}
...
for (const row of fixture.dashboard) {
  ...
  ok(
    `${row.name}: set volume status`,
    s.setStatus === row.setStatus,
    `got ${s.setStatus}, sheet says ${row.setStatus}`,
  );
}
...
if (planStats) {
  ...
  const flatStats = M.allStats(SEED.exercises, SEED.entries, flat, TODAY).find(
    (s) => s.exercise.name === fixture.plan.exercise,
  );
  ...
  const verdictMap = {
    "Ideal step": "ideal",
    Stretch: "stretch",
    "Too big a jump": "toobig",
    "Already beaten": "beaten",
  };
  ok(
    "planner: verdict",
    plan.band.key === verdictMap[fixture.plan.verdict],
    `got ${plan.band.key}, sheet says ${fixture.plan.verdict}`,
  );

  // Both grids, cell by cell. The sheet only goes to 5 sets; ours adds a 6th,
  // and it stopped at 12 reps, where ours carries on into the size range — so
  // only the rows the sheet actually had are checked against it.
  ok(
    "the sheet rows still lead the axis, in the sheet order",
    M.REP_SCHEMES.slice(0, M.SHEET_REP_SCHEMES).join() === "3,4,5,6,8,10,12",
    M.REP_SCHEMES.join(),
  );
  for (const [ri, reps] of M.REP_SCHEMES.slice(
    0,
    M.SHEET_REP_SCHEMES,
  ).entries()) {
    for (let ci = 0; ci < 5; ci++) {
      ...
      close(
        `grid weight ${reps}r x ${ci + 1}s`,
        cell.weight,
        fixture.plan.weightGrid[ri][ci],
      );
      close(
        `grid score ${reps}r x ${ci + 1}s`,
        cell.score,
        fixture.plan.scoreGrid[ri][ci],
        0.0001,
      );
    }
  }
...

{
  ...
  const withBar = {
    ...planStats,
    base: 20,
    exercise: { ...planStats.exercise, base: 20 },
  };
  ...
  ok(
    "every grid weight sits on the ladder",
    offLadder === 0,
    `${offLadder} off-ladder`,
  );
  ok("no grid weight is under the bar", underBar === 0, `${underBar} under`);
  ok(
    "every grid option still meets the target",
    shortOfTarget === 0,
    `${shortOfTarget} short`,
  );
  ok(
    "the prescribed weight sits on the ladder",
    Math.abs((plan.weight - 20) % 2.5) < 1e-9,
    String(plan.weight),
  );
  ...
  const heavy = {
    ...planStats,
    base: 200,
    exercise: { ...planStats.exercise, base: 200 },
  };
...

{
  ...
  close(
    "fatigue on the day is the peak",
    M.fatigueAt(0, 1, rs),
    rs.fatiguePeak,
    1e-9,
  );
  close(
    "fatigue falls by 1/e after one tau",
    M.fatigueAt(rs.fatigueTau, 1, rs),
    rs.fatiguePeak / Math.E,
    1e-9,
  );
  ok(
    "fatigue decays monotonically",
    M.fatigueAt(1, 1, rs) > M.fatigueAt(2, 1, rs),
  );
  ok("fatigue is spent after a fortnight", M.fatigueAt(14, 1, rs) === 0);
  ok(
    "a brutal session cannot exceed the cap",
    M.fatigueAt(0, 100, rs) <= 0.15 + 1e-12,
  );
  ok(
    "the ready line falls between day 1 and day 2",
  ...
  ok(
    "severity is bounded",
    sev(50, 0) <= 1.8 + 1e-12 && sev(1, 10) >= 0.4 - 1e-12,
  );

  // Accrual: earned by the week, and only while the rest is productive.
  close(
    "half a week earns half the gain",
    M.accrualAt(3.5, 0.01, 10),
    0.005,
    1e-12,
  );
  close("a week earns the week", M.accrualAt(7, 0.01, 10), 0.01, 1e-12);
  close(
    "past the window earns nothing more",
    M.accrualAt(40, 0.01, 10),
    (0.01 * 10) / 7,
    1e-12,
  );
  ...
  close(
    "nothing is lost at the grace boundary",
    M.retentionAt(rs.graceDays, rs.graceDays, rs),
    1,
    1e-12,
  );
  close(
    "nothing is lost before it",
    M.retentionAt(3, rs.graceDays, rs),
    1,
    1e-12,
  );
  close(
    "one half-life loses half the losable part",
    M.retentionAt(rs.graceDays + rs.detrainHalfLife, rs.graceDays, rs),
    rs.retainedFloor + (1 - rs.retainedFloor) / 2,
    1e-12,
  );
  close(
    "a decade off leaves exactly the floor",
    M.retentionAt(3650, rs.graceDays, rs),
    rs.retainedFloor,
    1e-9,
  );
  ...
  ok(
    "a month off costs a few percent",
    lost(28) > 3 && lost(28) < 8,
    `${lost(28).toFixed(1)}%`,
  );
  ok(
    "two months off costs low double digits",
    lost(56) > 9 && lost(56) < 18,
    `${lost(56).toFixed(1)}%`,
  );

  // The typical gap is the median of the gaps, not of the days.
  close(
    "typical interval, an odd number of gaps",
    M.typicalInterval([{ day: 0 }, { day: 3 }, { day: 7 }, { day: 10 }]),
    3,
  );
  close(
    "typical interval, an even number of gaps",
    M.typicalInterval([
      { day: 0 },
      { day: 3 },
      { day: 7 },
  ...
  close(
    "typical interval is a median, not a mean",
    M.typicalInterval([{ day: 0 }, { day: 3 }, { day: 6 }, { day: 90 }]),
    3,
  );
  ok(
    "two sessions are not enough to call a rhythm",
    M.typicalInterval([{ day: 0 }, { day: 3 }]) === null,
  );
}

{
  ...
  ok(
    "a repeat on the day is still recovering",
    d0.readiness.phase.key === "recovering",
  );
  ok(
    "a day later is still recovering",
    d1.readiness.phase.key === "recovering",
  );
  ...
  ok(
    "a same-day repeat asks for less than the session it follows",
    d0.nextTarget < d0.lastAdj,
    `${d0.nextTarget} vs ${d0.lastAdj}`,
  );
  ok(
    "the target climbs as the fatigue clears",
    d0.nextTarget < d1.nextTarget && d1.nextTarget < d3.nextTarget,
  );
  close(
    "a week off earns exactly the weekly gain",
    d7.nextTarget,
    d7.baseTarget,
    0.0001,
  );
  ok(
    "rest past the window stops adding",
    at(10).nextTarget === at(13).nextTarget,
  );
  ok(
    "a layoff asks for less than you last did",
    d90.nextTarget < d90.lastAdj,
    String(d90.nextTarget),
  );
  ok(
    "the target falls the longer the layoff runs",
    at(180).nextTarget < d90.nextTarget,
  );

  // Detraining discounts your best; fatigue does not.
  close(
    "fatigue leaves your best alone",
    d1.readiness.currentBest,
    d1.bestAdj,
    1e-9,
  );
  ok("a layoff discounts your best", d90.readiness.currentBest < d90.bestAdj);
  close(
    "the discount is the retention",
    d90.readiness.currentBest,
    d90.bestAdj * d90.readiness.retention,
    1e-9,
  );
  ...
  for (const d of [0, 1, 7, 90]) {
    ...
    close(
      `model off, day ${d}: the flat step`,
      off.nextTarget,
      off.baseTarget,
      1e-9,
    );
    ok(
      `model off, day ${d}: your best stands`,
      off.readiness.currentBest === off.bestAdj,
    );
  }
  ...
  const hardR = M.exerciseStats(
    ex,
    hard,
    settings,
    M.isoAddDays(anchor, 1),
  ).readiness;
  const easyR = M.exerciseStats(
    ex,
    easy,
    settings,
    M.isoAddDays(anchor, 1),
  ...
  ok(
    "eight sets to failure costs more than three easy ones",
    hardR.fatigue > easyR.fatigue,
  );
  ...
  ok(
    "a comeback plan is measured against today’s best",
    backPlan.bestNow === back.readiness.currentBest,
  );
  ok(
    "a comeback plan is lighter than the last session",
    backPlan.weight < 100,
    String(backPlan.weight),
  );
  ok(
    "a comeback plan is called a way back in",
    backPlan.band.key === "return",
    backPlan.band.key,
  );
  ok(
    "the way-back-in verdict says so",
    /way back in/i.test(M.plainVerdict(backPlan.band, -20)),
  );
  const overreach = backPlan.grid
    .flat()
    .filter((c) => c.weight < 100 - 1e-9 && c.band.key === "toobig");
  ok(
    "no weight under the last session reads as too big a jump",
    overreach.length === 0,
  ...
  ok(
    "options over the last weight are judged normally",
    shortLayoff.filter((c) => c.weight >= 100).length > 0 &&
      shortLayoff
        .filter((c) => c.weight >= 100)
        .every((c) => c.band.key !== "return"),
  );
  ok(
    "options under it are the way back in",
    shortLayoff
      .filter((c) => c.weight < 100)
      .every((c) => c.band.key === "return"),
  );
  ...
  ok(
    "the way-back-in band is only for a real layoff",
    M.planFor(at(5), settings, {})
      .grid.flat()
      .every((c) => c.band.key !== "return"),
  );
  ...
  for (const d of [0, 1, 2, 5, 14, 30, 120]) {
    ...
    ok(
      `day ${d}: the weight still meets its target`,
      pl.score >= pl.target - 1e-9,
      `${pl.score} vs ${pl.target}`,
    );
    ok(
      `day ${d}: the weight is loadable`,
      Math.abs((pl.weight / pl.step) % 1) < 1e-9,
    );
  }
  ...
  const slowStats = M.exerciseStats(
    ex,
    slow,
    settings,
    M.isoAddDays(anchor, 42 + 15),
  );
  close(
    "the rhythm is read off the log",
    slowStats.readiness.typicalInterval,
    14,
  );
  ok(
    "the grace period stretches to the rhythm",
    slowStats.readiness.grace >= 28,
  );
  ok(
    "day fifteen of a fortnightly lift is not a layoff",
    slowStats.readiness.phase.key !== "detrained",
  );
}
...
if (planStats) {
  const forced = M.planFor(planStats, settings, {
    target: 120,
    reps: 5,
    sets: 3,
  });
  ...
  ok(
    "override weight clears the target",
    forced.weight * M.repFactor(5, "epley") * M.setBonus(3, 0.05) >= 120 - 1e-9,
  );
  ok(
    "override weight is loadable",
    Math.abs((forced.weight / forced.step) % 1) < 1e-9,
  );
}
...
close("snapReps 20 -> 20", M.snapReps(20), 20); // on the axis now
close("snapReps 18 -> 16", M.snapReps(18), 16);
close("snapReps 40 -> 20", M.snapReps(40), 20); // still clamps at the top

// Bands, at the edges.
const st = { stretchBand: 0.03, idealBand: 0.01 };
ok(
  "band: at the ideal ceiling is still ideal",
  M.bandFor(101, 100, 50, st).key === "ideal",
);
ok(
  "band: just over the ideal ceiling is a stretch",
  M.bandFor(101.5, 100, 50, st).key === "stretch",
);
ok(
  "band: over the stretch ceiling is too big",
  M.bandFor(103.5, 100, 50, st).key === "toobig",
);
ok(
  "band: at or below the best is beaten",
  M.bandFor(100, 100, 100, st).key === "beaten",
);
ok(
  "band: beaten outranks too big",
  M.bandFor(110, 100, 120, st).key === "beaten",
);
...
ok(
  "relativeDate today",
  M.relativeDate("2026-08-18", "2026-08-18") === "Today",
);
ok(
  "relativeDate yesterday",
  M.relativeDate("2026-08-17", "2026-08-18") === "Yesterday",
);
ok(
  "isoAddDays crosses a month",
  M.isoAddDays("2026-08-31", 1) === "2026-09-01",
);
...
// --- logSet: the merge rule ---
{
  ...
  const rows = store
    .entriesOn(ex, day)
    .filter((e) => e.weight === 100 && e.reps === 5);
  ok(
    "logSet merges two identical sets into one block",
    rows.length === 1,
    `got ${rows.length} rows`,
  );
  ok(
    "a merged block counts both sets",
    rows[0].sets === 2,
    `sets=${rows[0]?.sets}`,
  );
  ok(
    "a merged block keeps the hardest RIR",
    rows[0].rir === 1,
    `rir=${rows[0]?.rir}`,
  );
  ok(
    "logSet did not disturb the rest of the day",
    store.entriesOn(ex, day).length === before + 1,
  );

  store.logSet({ exerciseId: ex, date: day, weight: 100, reps: 4, rir: 0 });
  const four = store
    .entriesOn(ex, day)
    .filter((e) => e.weight === 100 && e.reps === 4);
  ok(
    "a different rep count becomes its own block",
    four.length === 1 && four[0].sets === 1,
  );
}
...
// --- logSet: notes are collected, not lost ---
{
  ...
  const row = store
    .entriesOn(ex, "2026-08-18")
    .find((e) => e.weight === 60 && e.reps === 5);
  ok(
    "a merged block collects both notes",
    row.notes === "felt light; grip slipped",
    row.notes,
  );
  store.logSet({
    exerciseId: ex,
  ...
  const again = store
    ...
    .find((e) => e.weight === 60 && e.reps === 5);
  ok(
    "a repeated note is not duplicated",
    again.notes === "felt light; grip slipped",
    again.notes,
  );
}
...
// --- undo: the merge branch restores every field it touched ---
{
  ...
  ok(
    "undo restores the block set count",
    back.sets === snap.sets,
    `${back.sets} vs ${snap.sets}`,
  );
  ok(
    "undo restores the block RIR",
    back.rir === snap.rir,
    `${back.rir} vs ${snap.rir}`,
  );
  ok(
    "undo restores the block notes",
    back.notes === snap.notes,
    `${back.notes} vs ${snap.notes}`,
  );
}
...
// --- undo: the create branch removes the entry it added ---
{
  ...
  store.undo();
  ok(
    "undo removes the entry logSet created",
    store.getEntries().length === before,
  );
  ok(
    "undo leaves no trace of the set",
    !store.getEntries().some((e) => e.weight === 123.5),
  );
}
...
// --- removeLastSet, both branches, and their inverses ---
{
  ...
  ok(
    "removeLastSet decrements a multi-set block",
    store.entriesOn(ex, day).find((e) => e.weight === 80).sets === 1,
  );
  store.undo();
  ok(
    "undo puts the removed set back",
  ...
  store.removeLastSet(ex, day);
  ok(
    "removeLastSet deletes a one-set block",
    store.getEntries().length === count,
  );
  store.undo();
  ok(
    "undo restores a deleted one-set block",
    store.getEntries().length === count + 1,
  );
  ok(
    "the restored block is the one that went",
    store.getEntries().some((e) => e.weight === 77.5 && e.reps === 3),
  );
}
...
// --- deleteEntry restores in place, not at the end ---
{
  ...
  ok(
    ...
    !store.getEntries().some((e) => e.id === victim.id),
  );
  store.undo();
  ok(
  ...
  ok(
    ...
    store
      .getEntries()
      .indexOf(store.getEntries().find((e) => e.id === victim.id)) === idx,
  );
}
...
// --- updateEntry and its inverse ---
{
  ...
  ok(
    "updateEntry applies the patch",
    hit.weight === 999 && hit.reps === 2 && hit.sets === 9,
  );
  store.undo();
...
// --- settings, order, and the coarse whole-document path ---
{
  ...
  ok(
    ...
    store.getSettings().lookbackDays === was,
  );
  ...
  store.undo();
  ok(
    "undo restores the whole log from a coarse snapshot",
    store.getEntries().length === n,
  );
}
...
// --- normalise: the guards that protect the log ---
{
  ...
  ok(
    "an entry for an unknown lift is dropped",
    !kept.some((e) => e.exerciseId === "ex-does-not-exist"),
  );
...
// --- reload(): another tab wrote, and this one must not lose its footing ---
{
  ...
  const outside = JSON.parse(
    globalThis.localStorage.getItem("liftingTracker.v1"),
  );
  ...
  store.reload();
  ok(
    "reload picks up the other tab’s write",
    !store.getEntries().some((e) => e.weight === 111),
  );
  ok(
    "reload drops an undo stack that no longer applies",
    store.canUndo() === false,
  );
}
...
// --- a long log must not blow the call stack when seq is derived ---
{
  ...
  let threw = null;
  try {
    store.importJSON(JSON.stringify(big));
  } catch (err) {
    threw = err;
  }
  ok(
    "a 200k-entry log normalises without a RangeError",
    threw === null,
    String(threw && threw.message),
  );
  ok(
    "seq is derived correctly at that size",
    store.getDoc().seq === 200001,
    String(store.getDoc().seq),
  );
  fresh();
}
...
// numbers, so they must still see them without touching anything.
{
  ...
  ok(
    "a detailed-view user keeps their numbers open",
  ...
  ok(
    "a simple-view user starts with them folded",
  ...
  ok(
    "a pre-split document keeps everything it used to show",
  ...
  ok(
    "an explicit choice beats the migration",
    store.importJSON(JSON.stringify(explicit)).settings.numbersOpen === false,
  );

  ok(
    "the retired setting is not carried forward",
  ...
  ok(
    "the setting is a boolean, not a coerced number",
    store.getSettings().numbersOpen === true,
  );
...
// --- readinessNote / readinessMaths, split out of one forked function ---
{
  ...
  ok(
    "the plain verdict keeps the jargon out",
    !/e1RM|deficit|retention/i.test(plain),
    plain,
  );
  ok(
    "both are empty when the model is off",
    M.readinessNote({ enabled: false }) === "" &&
      M.readinessMaths({ enabled: false }) === "",
  );
  ok(
    "both are empty without a last session",
    M.readinessNote({ enabled: true, days: null }) === "" &&
...
// directly rather than assumed.
{
  fresh();
  ok(
    "the document declares schema 2",
    store.getDoc().schema === 2,
    String(store.getDoc().schema),
  );

  const cfg = store.getSettings();
  const withLog = M.allStats(
    store.getExercises(),
    store.getEntries(),
    cfg,
    TODAY,
  ...
  const published = (list) =>
    list.map((s) => {
      ...
      row.readiness = s.readiness && {
        fatigue: s.readiness.fatigue,
        accrual: s.readiness.accrual,
        retention: s.readiness.retention,
        factor: s.readiness.factor,
        target: s.readiness.target,
        phase: s.readiness.phase.key,
        severity: s.readiness.severity,
      };
      row.plan = (() => {
        const p = M.planFor(s, cfg, {});
        return p.ready ? [p.weight, p.score, p.band.key] : null;
      })();
      return row;
    });
  ok(
    "stripping the per-set log changes not one published number",
...
// --- a v1 document backfills honestly rather than inventing detail ---
{
  ...
  ok(
    "a pre-v2 block gets one record per set",
    e && e.log.length === 3,
    `${e && e.log.length}`,
  );
  ok(
    "backfilled records carry no timestamp",
    e.log.every((r) => r.at === null),
  );
  ok(
    "backfilled records carry no per-set RIR",
    e.log.every((r) => r.rir === null),
...
// --- logging set by set is the one path that knows when a set happened ---
{
  ...
  const row = store
    .entriesOn(ex, day)
    .find((x) => x.weight === 100 && x.reps === 5);

  ok("the block still merges to one row", row.sets === 2);
  ok("the block still keeps the hardest RIR", row.rir === 1);
  ok(
    "but the log keeps each set is own RIR",
    row.log.map((r) => r.rir).join() === "3,1",
    row.log.map((r) => r.rir).join(),
  );
  ok(
    "every logged set is timestamped",
    row.log.every((r) => Number.isFinite(r.at) && r.at > 0),
  );
  ...
  ok(
    "undo takes the log record off with the set",
    back.sets === 1 && back.log.length === 1,
  );
...
// --- the log has to keep describing the block it belongs to ---
{
  ...
  ok(
    "the padding is honest about being padding",
    row.log.slice(1).every((r) => r.at === null && r.rir === null),
  );
  ok(
    "and the measured set survives the edit",
    Number.isFinite(row.log[0].at) && row.log[0].rir === 2,
  );
  ...
  store.undo();
  ok(
    "undo restores the log it truncated",
...
// --- undo must never hand back a shared array ---
{
  ...
  ok(
    "deleting and undoing restores the log",
    back && back.log.length === 1 && back.log[0].rir === 4,
  );
  ...
  store.undo();
  ok(
    "the restored log is a copy, not a shared reference",
...
// --- junk in the log is discarded rather than trusted ---
{
  ...
  target.log = [
    ...
    { at: 1, rir: 1 },
    { at: 2, rir: 1 }, // more records than there are sets
  ];
  const back = store
    .importJSON(JSON.stringify(doc))
    .entries.find((e) => e.id === target.id);
  ok(
    "the log is trimmed to the set count",
    back.log.length === 3,
    String(back.log.length),
  );
  ok('a nonsense timestamp becomes "not measured"', back.log[0].at === null);
  ok(
    'a nonsense RIR becomes "not known"',
    back.log[0].rir === null || Number.isNaN(back.log[0].rir),
  );
  ...
  ok(
    ...
    back.log[1].at === 1755590400000 && back.log[1].rir === 2,
...
// --- a v1 backup still imports, and an old client can still read v2 ---
{
  ...
  ok(
    "and comes back with a log",
    after.entries.every((e) => e.log.length === e.sets),
  );
  ...
  ok(
    "every field an older build reads is still there",
...
// --- the CSV gains timing without changing its shape ---
{
  ...
  ok(
    "the original CSV columns are untouched",
    head.slice(0, 7).join() === "Date,Exercise,Weight (kg),Reps,Sets,RIR,Notes",
    head.slice(0, 7).join(),
  );
  ok(
    "and timing is added on the end",
    head.slice(7).join() === "Started,Finished,Median rest (s)",
    head.slice(7).join(),
  );
  ok(
  ...
  ok(
    "a measured set reports when it started",
    /,\d{2}:\d{2},/.test(measured),
    measured,
  );
}
...

{
  ...
  ok(
    "the memo agrees with metrics.js on every published field",
  ...
  for (const [i, st] of memo.entries()) {
    close(
      `memo lastAdj matches for ${st.exercise.name}`,
      st.lastAdj ?? 0,
      raw[i].lastAdj ?? 0,
      1e-12,
    );
    close(
      `memo nextTarget matches for ${st.exercise.name}`,
      st.nextTarget ?? 0,
      raw[i].nextTarget ?? 0,
      1e-12,
    );
    close(
      `memo trendPerWeek matches for ${st.exercise.name}`,
      st.trendPerWeek ?? 0,
      raw[i].trendPerWeek ?? 0,
      1e-12,
    );
  }
  ...
  ok(
    "a different day drops the cache",
    select.allStats(cfg, "2026-08-19") !== onDay,
  );
}
...
// --- the band settings invalidate plans, and only plans ---
{
  ...
  ok(
    "a repeat plan is served from the cache",
    select.planFor(st, cfg, {}, day) === p1,
  );
  ok(
    "a different override computes a different plan",
    select.planFor(st, cfg, { reps: 8 }, day) !== p1,
  );
  ok(
    "the same override is cached again",
    select.planFor(st, cfg, { reps: 8 }, day) ===
      select.planFor(st, cfg, { reps: 8 }, day),
  );

  const wide = { ...cfg, idealBand: 0.2 };
  ok(
    "widening the ideal band keeps the stats cache",
  ...
  ok(
    "and the verdict actually moves with it",
    p2.band.key !== p1.band.key || p1.band.key === "beaten",
  );
...
// --- a write must invalidate everything ---
{
  ...
  ok(
    "and the new set is in the fresh stats",
    mine.entries.some((e) => e.weight === 500),
  );
  store.undo();
  ok("undoing drops it again", select.allStats(cfg, day) !== after);
  ok(
    "and the set is gone from the stats",
    !select
      .allStats(cfg, day)
      .find((s) => s.exercise.id === ex)
      .entries.some((e) => e.weight === 500),
  );
}
...
// --- the lazy grid must be exactly the grid that used to be built eagerly ---
{
  ...
  ok(
    "the grid is enumerable, as it was when it was a plain array",
    Object.keys(plan).includes("grid"),
  );
  ok(
    "the grid has a row per rep scheme",
    plan.grid.length === M.REP_SCHEMES.length,
  );
  ok(
    "the grid has a column per set count",
    plan.grid[0].length === M.SET_COLUMNS.length,
  );
  ok("a second read is the same array", plan.grid === plan.grid);
  ok(
    "gentlest resolves without forcing the grid",
  ...
  const expect = col.length
    ? col.reduce((a, b) => (b.score < a.score ? b : a))
    : null;
  close(
    "gentlest picks the same cell the grid would",
    plan.gentlest?.score ?? 0,
    expect?.score ?? 0,
    1e-12,
  );
  ok(
    "gentlest picks the same rep scheme",
    (plan.gentlest?.reps ?? 0) === (expect?.reps ?? 0),
  );
}
...
function block(o) {
  ...
  return {
    id: o.id ?? "e1",
    exerciseId: o.exerciseId ?? "x",
    date: o.date ?? "2026-08-18",
    weight: o.weight ?? 100,
    reps: o.reps ?? 5,
    sets,
    rir: o.rir ?? null,
...
// --- ordering: the reason the timestamps exist at all ---
{
  ...
  ok(
    "sessionSets flattens to one record per physical set",
    sets.length === 3,
    String(sets.length),
  );
  ok(
    "and puts them in the order they actually happened",
    sets.map((s) => s.weight).join() === "100,60,100",
    sets.map((s) => s.weight).join(),
  );
  ok("each set carries its own RIR", sets.map((s) => s.rir).join() === "3,4,2");
  ok(
    "measured sets are marked as measured",
    sets.every((s) => s.measured),
  );
}

{
  ...
  ok(
    "and none of them claims to be measured",
    sets.every((s) => !s.measured && s.at === null),
  );
  ok("hasTiming is false without timestamps", I.hasTiming(sets) === false);
  ok(
    "hasPerSetRir is false without per-set RIR",
    I.hasPerSetRir(sets) === false,
  );
}

// --- rest ---
{
  const sets = I.sessionSets([
    block({ log: [rec(0), rec(2), rec(4.5), rec(60), rec(62)] }),
  ]);
  const gaps = I.restIntervals(sets).map((r) => r.seconds);
  ok(
    "rest is the gap between consecutive sets",
    gaps.join() === "120,150,3330,120",
    gaps.join(),
  );
  ok(
    "typicalRest is the median of the plausible gaps",
    I.typicalRest(sets) === 120,
    String(I.typicalRest(sets)),
  );
  ok("a 55-minute gap is not counted as rest", I.typicalRest(sets) < 1200);
  ok(
    "sessionDuration spans first to last",
    I.sessionDuration(sets) === 62 * 60,
  );

  const mixed = I.sessionSets([block({ log: [rec(0), rec(null), rec(4)] })]);
  ok(
    "a backfilled set breaks the chain rather than inventing a gap",
    I.restIntervals(mixed).length === 0,
    JSON.stringify(I.restIntervals(mixed)),
  );
  ok(
    "typicalRest says nothing when it knows nothing",
    I.typicalRest(mixed) === null,
  );
  ok(
    "sessionDuration still spans the two it did measure",
    I.sessionDuration(mixed) === 240,
  );
  ok(
    "one measured set is not a duration",
    I.sessionDuration(I.sessionSets([block({ log: [rec(0)] })])) === null,
  );
}
...
{
  const sets = I.sessionSets([
    block({ weight: 100, reps: 5, log: [rec(0, 3), rec(3, 2), rec(6, 0)] }),
  ]);
  ...
  ok(
    "the rest before each later set is measured",
    tl[1].restBefore === 180 && tl[2].restBefore === 180,
  );
  close(
    "each set carries its own e1RM",
    tl[0].e1rm,
    M.e1rm(100, 5, settings.formula),
    1e-9,
  );
}
...
// --- the fade inside a session ---
{
  ...
  ok(
    "reps falling away shows as a negative slope",
    fade.repDrop < 0,
    String(fade.repDrop),
  );
  ok(
    "RIR closing on zero shows as a negative slope",
    fade.rirDrop < 0,
    String(fade.rirDrop),
  );
  ok("and the fit is flagged reliable at four sets", fade.reliable === true);

  const steady = I.sessionTimeline(
    I.sessionSets([block({ reps: 5, log: [rec(0, 2), rec(3, 2), rec(6, 2)] })]),
    settings,
  );
  close(
    "a steady session has no rep drop",
    I.withinSessionFade(steady).repDrop,
    0,
    1e-9,
  );

  const thin = I.sessionTimeline(
    I.sessionSets([block({ log: [rec(0, 2), rec(3, 1)] })]),
    settings,
  );
  ok(
    "two sets is not a trend",
    I.withinSessionFade(thin).repDrop === null && thin.length === 2,
  );
...
// --- severity, read off the real trajectory instead of the block minimum ---
{
  ...
  const sets = I.sessionSets([
    block({
      sets: 5,
      log: [rec(0, 4), rec(3, 3), rec(6, 2), rec(9, 1), rec(12, 0)],
    }),
  ]);
  ...
  ok(
    "the block minimum overstates a session that faded to failure",
    v1 > v2,
    `${v1} vs ${v2}`,
  );
  close(
    "the mean RIR of 2 is what severity should see",
    v2,
    1.25 - 0.12 * 2,
    1e-9,
  );

  const allHard = I.sessionSets([
    block({ sets: 3, log: [rec(0, 0), rec(3, 0), rec(6, 0)] }),
  ]);
  ok(
    "a session that really was all-out still scores high",
    I.sessionSeverityV2(allHard, { setsPerSession: 3 }) > v2,
  );
  ok(
    "no per-set RIR falls back to a normal session",
    I.sessionSeverityV2(I.sessionSets([block({ sets: 3 })]), {
      setsPerSession: 3,
    }) === 1,
  );
  ok(
    "severity stays inside its bounds",
...
{
  ok(
    "under a hundred, milestones are every 5 kg",
    I.milestoneStep(60) === 5 && I.milestoneStep(99) === 5,
  );
  ok(
    "past a hundred, every 10",
    I.milestoneStep(100) === 10 && I.milestoneStep(199) === 10,
  );
...

{
  ...
  ok(
    "the next milestone on the bar is the next round number up",
    w.value === 100,
    String(w.value),
  );
  ok("it is measured from your best, not your last", w.from === 97.5);
  ok(
    "the next score milestone rounds by its own magnitude",
    e.value === 120,
    String(e.value),
  );
  ok(
    "a lift with no history has no milestone",
...
// --- a hand-set milestone stands in for the guess ---
{
  const targeted = {
    ...fakeStats,
    exercise: { ...fakeStats.exercise, milestone: { value: 120, reps: 3 } },
  };
  const manual = I.manualMilestone(targeted);
  ok(
    "a manual target is picked up",
    manual && manual.value === 120 && manual.manual === true,
  );
  ok("it carries its own rep count", manual.reps === 3);
  close(
    "measured from your current best",
    manual.from,
    I.bestLoad(fakeStats),
    1e-9,
  );

  const ms = I.milestones(targeted);
  ok(
    "milestones() uses the manual target in place of the auto guess",
    ms.filter((m) => m.kind === "weight").length === 1 &&
      ms.find((m) => m.kind === "weight").value === 120,
  );
  ok(
    "the score milestone is untouched by the weight override",
  ...
  close(
    "and converts it at the manual reps, not stats.lastReps",
    r.scoreNeeded,
    120 * M.repFactor(3, settings.formula) * M.setBonus(3, settings.setBonusK),
    1e-9,
  );

  const inert = {
    ...fakeStats,
    exercise: { ...fakeStats.exercise, milestone: { value: null, reps: 8 } },
  };
  ok(
    "a reps-only entry with no value is inert — order of typing does not matter",
    I.manualMilestone(inert) === null,
  );
  ok(
    "so milestones() falls back to the auto guess",
    I.milestones(inert).find((m) => m.kind === "weight").value === 100,
  );

  const cleared = {
    ...fakeStats,
    exercise: { ...fakeStats.exercise, milestone: null },
  };
  ok("clearing it reverts to automatic", I.manualMilestone(cleared) === null);
  ok(
    "and milestones() goes back to the round-number guess",
...

{
  ...
  ok(
    "an ETA counts the days at the fitted rate",
    eta.days === 50,
    String(eta.days),
  );
  ok(
    "and turns them into a date",
    eta.date === M.isoAddDays(TODAY, 50),
    eta.date,
  );
  ok(
    "and into sessions, at your own training rhythm",
    eta.sessions === 14,
    String(eta.sessions),
  );

  ok(
    "a target already passed reads as reached",
    I.etaTo(100, fakeStats, { todayIso: TODAY }).reached === true,
  );
  ok(
    "a provisional fit publishes no ETA at all",
    I.etaTo(
      120,
      { ...fakeStats, trendReliable: false },
      { todayIso: TODAY },
    ) === null,
    "an ETA off two points is a guess wearing a date",
  );
  ok(
    "a lift going nowhere publishes no ETA",
    I.etaTo(120, { ...fakeStats, trendPerDay: 0 }, { todayIso: TODAY }) ===
      null,
  );
  ok(
    "a lift going backwards publishes no ETA",
    I.etaTo(120, { ...fakeStats, trendPerDay: -0.1 }, { todayIso: TODAY }) ===
      null,
  );
  const far = I.etaTo(400, fakeStats, { todayIso: TODAY });
  ok(
    "an absurd horizon is flagged rather than dated",
    far.tooFar === true && far.date === null,
  );
}

{
  ...
  ok(
    "the runway aims at the next round number on the bar",
    r.milestone.value === 100,
  );
  close(
    "converted into what that weight would score at your reps and sets",
    r.scoreNeeded,
    100 * M.repFactor(5, settings.formula) * M.setBonus(3, settings.setBonusK),
    1e-9,
  );
  ...
  ok(
    "a lift with no history has no runway",
    I.runway({ ...fakeStats, entries: [] }, settings) === null,
  );
}
...
// --- the fatigue model, drawn rather than reported ---
{
  ...
  ok(
    "the curve runs a point per day",
    curve.length === 31,
    String(curve.length),
  );
  ok(
    "fatigue is highest on the day of the session",
    curve[0].fatigue >= curve[1].fatigue,
  );
  ok("fatigue decays to nothing", curve[14].fatigue === 0);
  ok("the target climbs as fatigue clears", curve[7].target > curve[0].target);
  ok(
    "the curve knows when the lift is fit again",
    curve[0].recovered === false && curve[curve.length - 1].recovered === true,
  );
  close(
    "day zero agrees with the model on the card",
    curve[0].factor,
    M.readinessFor({ ...st, daysSince: 0 }, cfg).factor,
    1e-9,
  );
  ok(
    "with the model off there is no curve",
    I.readinessCurve(st, { ...cfg, readiness: "off" }, { days: 10 }).length ===
      0,
  );
}
...

{
  ...
  const inWindow = all
    .flatMap((s) => s.entries)
    .filter(
      (e) => e.day > M.dayNumber(TODAY) - 84 && e.day <= M.dayNumber(TODAY),
    );
  close(
    "and account for every set inside the window without double counting",
    tons.reduce((n, b) => n + b.sets, 0),
    inWindow.reduce((n, e) => n + (Number(e.sets) > 0 ? Number(e.sets) : 1), 0),
    1e-9,
  );

  const prs = I.prTimeline(all);
  ok(
    "the PR timeline is in date order",
    prs.every((p, i) => i === 0 || prs[i - 1].day <= p.day),
  );
  ok(
    "every PR names its lift",
    prs.every((p) => typeof p.name === "string" && p.name.length),
  );
  ok(
    "every PR after the first says what it beat",
    prs.filter((p) => p.delta !== null).every((p) => p.delta > 0),
  );
  close(
    "the count matches what exerciseStats marked",
    prs.length,
    all.reduce((n, s) => n + s.prCount, 0),
    1e-9,
  );
  ...
  ok(
    "a day with no training is a zero, not a gap",
    grid.every((g) => Number.isFinite(g.sets)),
  );
  ...
  ok(
    ...
    bal.every((r, i) => i === 0 || bal[i - 1].sets >= r.sets),
  );
  close(
    "the shares add up to one",
    bal.reduce((n, r) => n + r.share, 0),
    bal.some((r) => r.sets) ? 1 : 0,
    1e-9,
  );
  ok(
    "a lift with a weekly target gets a verdict",
    bal
      .filter((r) => r.target)
      .every((r) => ["under", "over", "on target"].includes(r.status)),
  );
}
...
// one without knowing it is one.
{
  ...
  close(
    "a reps set is worth its reps, with the set bonus",
    st.lastAdj,
    12 * M.setBonus(3, settings.setBonusK),
    1e-9,
  );
  ...
  close(
    "the fewest reps that clear a target",
    M.repsForTarget(12.85, 3, settings),
    13,
    1e-9,
  );
  ...
  ok(
    "and the answer really does clear it",
    need * M.setBonus(4, settings.setBonusK) >= 20 - 1e-9,
  );
  ok(
    "by the smallest whole rep",
    (need - 1) * M.setBonus(4, settings.setBonusK) < 20,
  );

  const plan = M.planFor(st, { ...settings, readiness: "off" }, {});
  ok(
    "the plan knows what kind of lift it is",
  ...
  close(
    "scored on the reps scale",
    plan.score,
    plan.reps * M.setBonus(plan.sets, settings.setBonusK),
    1e-9,
  );
  ...
  ok(
    "the choice is how many sets",
    plan.options.length === M.SET_COLUMNS.length,
  );
  ok(
    "each option clears the target",
    plan.options.every((o) => o.score >= plan.target - 1e-9),
    JSON.stringify(plan.options.map((o) => o.score)),
  );
  ok(
    "more sets never asks for more reps",
    plan.options.every((o, i) => i === 0 || o.reps <= plan.options[i - 1].reps),
    plan.options.map((o) => `${o.sets}x${o.reps}`).join(" "),
  );
  ok(
    "one option is the one currently picked",
    plan.options.filter((o) => o.isPick).length === 1,
  );
  ok(
    "the gentlest option overshoots less than the default",
  ...
  close(
    "last load is the reps, not the weight",
    M.lastSessionLoad(st),
    12,
    1e-9,
  );
  close(
    "and on a weighted lift it is still the weight",
    M.lastSessionLoad(
      M.exerciseStats(SEED.exercises[0], SEED.entries, settings, TODAY),
    ),
    M.lastSessionWeight(
      M.exerciseStats(SEED.exercises[0], SEED.entries, settings, TODAY),
    ),
  ...
  // Display helpers.
  ok(
    "a reps lift reads without a weight",
    M.describeSet(repsEx, 3, 12, 0) === "3 × 12 reps",
  );
  ok(
    "a weighted one still reads with one",
    M.describeSet({ name: "Squat" }, 3, 5, 100) === "3 × 5 @ 100 kg",
  );
  ok(
    "units follow the lift",
    M.loadUnit(repsEx) === "reps" && M.loadUnit({}) === "kg",
  );
  ...
  ok(
    "the next milestone is a rep count",
    !!rm && rm.value === 15,
    JSON.stringify(rm),
  );
  ok("measured from the best set ever done", rm.from === 12);
  ok(
    "there is no weight milestone on a reps lift",
    !ms.some((m) => m.kind === "weight"),
  );
  const run = I.runway(st, settings, { todayIso: TODAY });
  close(
    "the runway converts by the set bonus alone",
    run.scoreNeeded,
    15 * M.setBonus(run.sets, settings.setBonusK),
    1e-9,
  );
  ...
  const stTargeted = {
    ...st,
    exercise: { ...repsEx, milestone: { value: 20, reps: 99 } },
  };
  const manualReps = I.manualMilestone(stTargeted);
  ok(
    "a manual reps target ignores the reps field — the value IS the reps",
    manualReps && manualReps.kind === "reps" && manualReps.value === 20,
  );
  ok(
    "milestones() picks it up in place of the auto guess",
...

{
  ...
  ok(
    "and they get slower",
    I.LEVELS.every(
      (l, i) => i === 0 || l.gainPerWeek < I.LEVELS[i - 1].gainPerWeek,
    ),
  );
  ok(
    "and ask for more volume as they go",
    I.LEVELS.every(
      (l, i) => i === 0 || l.setsPerWeek >= I.LEVELS[i - 1].setsPerWeek,
    ),
  );
  ok(
    "every level explains itself",
    I.LEVELS.every((l) => l.hint.length > 20),
  );

  ok(
    "a lift set to a preset reports it",
    I.levelOf({ gainPerWeek: 0.015 }).key === "beginner",
  );
  ok(
    "a lift tuned by hand reports nothing rather than the nearest",
    I.levelOf({ gainPerWeek: 0.0075 }) === null,
    "the shipped default sits between two presets and must read as Custom",
  );

  ok(
    "1.31%/wk looks like a beginner",
    I.nearestLevel(0.0131).key === "beginner",
  );
  ok(
    "0.40%/wk looks intermediate",
    I.nearestLevel(0.004).key === "intermediate",
  );
  ok("0.10%/wk looks advanced", I.nearestLevel(0.001).key === "advanced");
  ok(
    "no measurable gain falls to the slowest",
    I.nearestLevel(0).key === "advanced",
  );

  const base = {
    exercise: { gainPerWeek: 0.005 },
    gainPerWeek: 0.005,
    trendReliable: true,
    lastAdj: 100,
    trendPerWeek: 1.31,
  ...
  ok(
    "a lift moving much faster than its setting says so",
    sug && sug.level.key === "beginner",
  );
  close("and reports the rate it measured", sug.measured, 0.0131, 1e-9);
  ok("with the window it measured over", sug.weeks === 14);

  ok(
    "a lift moving at roughly its setting says nothing",
    I.suggestLevel({ ...base, trendPerWeek: 0.52 }) === null,
  );
  ok(
    "a thin fit says nothing",
    I.suggestLevel({ ...base, trendReliable: false }) === null,
  );
  ok(
    "a lift going backwards says nothing",
    I.suggestLevel({ ...base, trendPerWeek: -0.4 }) === null,
  );
  ok(
    "a lift with no history says nothing",
    I.suggestLevel({ ...base, lastAdj: null }) === null,
  );
  ok(
    "and it never suggests what is already set",
    I.suggestLevel({
      ...base,
      exercise: { gainPerWeek: 0.015 },
      gainPerWeek: 0.015,
    }) === null,
  );
}
...
// --- the store keeps the kind, and defaults it safely ---
{
  ...
  ok(
    "and an ordinary one still is not",
    store.addExercise({ name: "Row" }).kind === "weight",
  );
  ok(
    "junk falls back to a weighted lift",
    store.addExercise({ name: "X", kind: "nonsense" }).kind === "weight",
  );
  ...
  ok(
    "a document written before any of this loads as weighted",
    store
      .importJSON(JSON.stringify(doc))
      .exercises.every((e) => e.kind === "weight"),
  );
}
...
// --- a hand-set milestone round-trips through the store ---
{
  ...
  ok(
    "the target is kept",
    withTarget.milestone.value === 60 && withTarget.milestone.reps === 5,
  );

  store.updateExercise(ex.id, { milestone: { value: 0, reps: 5 } });
  ok(
    "a zeroed value is dropped rather than kept as junk",
    store.getExercise(ex.id).milestone.value === null,
  );

  store.updateExercise(ex.id, { milestone: null });
  ok(
    "clearing it goes back to null, not a stale object",
  ...
  const reloaded = store
    .importJSON(JSON.stringify(doc2))
    .exercises.find((e) => e.id === ex.id);
  ok(
    "a milestone survives export and import",
    reloaded.milestone.value === 60 && reloaded.milestone.reps === 5,
  );

  const created = store.addExercise({
    name: "Front Squat",
    milestone: { value: 80, reps: 1 },
  });
  ok(
    "addExercise accepts a milestone up front",
    created.milestone.value === 80 && created.milestone.reps === 1,
  );
}
...
// --- free-form tags: trimmed, deduplicated, round-trip through the store ---
{
  ...
  ok(
    "no tags by default",
    Array.isArray(noTags.tags) && noTags.tags.length === 0,
  );

  const withTags = store.addExercise({
    name: "Barbell Row",
    tags: [" Pull ", "Back", "Pull", ""],
  });
  ok(
    "tags are trimmed, deduplicated and emptied of blanks",
    JSON.stringify(withTags.tags) === JSON.stringify(["Pull", "Back"]),
    JSON.stringify(withTags.tags),
  );

  store.updateExercise(noTags.id, { tags: ["Legs", "Push"] });
  ok(
    "tags can be set after the fact",
    JSON.stringify(store.getExercise(noTags.id).tags) ===
      JSON.stringify(["Legs", "Push"]),
  );

  store.updateExercise(noTags.id, { tags: [] });
  ok(
    "tags can be cleared back to none",
    store.getExercise(noTags.id).tags.length === 0,
  );

  const doc = JSON.parse(store.exportJSON());
  for (const e of doc.exercises) delete e.tags;
  ok(
    "a document written before tags existed loads with none, not a crash",
    store
      .importJSON(JSON.stringify(doc))
      .exercises.every((e) => Array.isArray(e.tags) && e.tags.length === 0),
  );
}
...
// --- a lift can be called finished, and it is a decision, not a set count ---
{
  ...
  store.markDone(ex.id, day);
  ok(
    "marking twice does not double the row",
    store.getDone().length === 1,
    String(store.getDone().length),
  );
  ...
  store.markDone(ex.id, day);
  ok(
    "toggle reports which way it went",
    store.toggleDone(ex.id, day) === false,
  );
  ...
  ok(
    "toggling a finished lift clears it again",
    store.toggleDone(ex.id, day) === false,
  );
}
...
// --- the marker is about the day, so the sets underneath it can still move ---
{
  ...
  store.logSet({ exerciseId: ex.id, date: day, weight: 100, reps: 5 });
  ok(
    "logging another set after finishing does not unmark it",
  ...
  store.removeLastSet(ex.id, day);
  ok(
    "and taking every set back does not either — it is a decision, not a count",
    store.isDone(ex.id, day) === true,
  );
  ok(
    "a different day is untouched",
    store.isDone(ex.id, "2026-08-19") === false,
  );
}
...
// --- markers belong to their lift and go when it does ---
{
  ...
  ok(
    "deleting a lift drops its markers",
    store.isDone(ex.id, "2026-08-18") === false,
  );
  ok(
    "and leaves everyone else alone",
    store.isDone(other.id, "2026-08-18") === true,
  );

  store.clearAll();
  ok(
    "clearing the log clears the markers with it",
    store.getDone().length === 0,
  );
}
...
// --- it survives a backup, and a document written before it existed loads ---
{
  ...
  ok(
    ...
    store
      .importJSON(JSON.stringify(doc))
      .done.some((d) => d.exerciseId === ex.id && d.date === "2026-08-18"),
  );

  delete doc.done;
  ok(
    "a document written before lifts could be finished loads with none",
    store.importJSON(JSON.stringify(doc)).done.length === 0,
  );

  const junk = JSON.parse(store.exportJSON());
  junk.done = [
    { exerciseId: "no-such-lift", date: "2026-08-18" },
    { exerciseId: ex.id },
    null,
  ];
  ok(
    "and junk markers are dropped rather than kept",
...
// --- what a finished session teaches the fatigue model ---
{
  ...
  ok(
    "with no finished sessions the setting stands",
    M.usualSets(null, ex) === 3 && M.usualSets([], ex) === 3,
  );
  ok(
    "two finished sessions are not yet a median worth trusting",
    M.usualSets([sess(5), sess(5)], ex) === 3,
  );
  ok("three are", M.usualSets([sess(5), sess(5), sess(5)], ex) === 5);
  ok(
    "and it is a median, not a mean",
    M.usualSets([sess(5), sess(5), sess(5), sess(30)], ex) === 5,
    String(M.usualSets([sess(5), sess(5), sess(5), sess(30)], ex)),
  );
  ok(
    "sessions never called done are not counted short",
    M.usualSets(
      [sess(5), sess(5), sess(5), { sets: 1 }, { sets: 1 }, { sets: 1 }],
      ex,
    ) === 5,
  );
  ok(
    "a lift with no setting at all falls back to three",
    M.usualSets(null, {}) === 3,
  );
  ...
  ok(
    "against a guess of three, five sets reads as harder than normal",
    guessed > 1.2,
    String(guessed),
  );
  ok(
    "against what this lift actually does, it reads as normal",
    Math.abs(measured - 1.01) < 0.02,
    String(measured),
  );
  ok(
    "and a genuinely big session still reads as one",
    M.sessionSeverity({ sets: 9, best: { rir: 0 } }, ex, [
      sess(5),
      sess(5),
      sess(5),
    ]) > measured,
...
// --- the stats carry the marker through to the views ---
{
  ...
  ok(
    "nothing is finished when no markers are handed over",
    plain.doneToday === false,
  );
  ok(
    "and no session claims to be",
    plain.sessions.every((x) => x.done === false),
  );

  const marked = M.exerciseStats(
    ex,
    entries,
    settings,
    anchor,
  ...
  ok(
    "yesterday being finished does not make today finished",
    later.doneToday === false,
  );
  ok(
    "but the session it belongs to still says so",
    later.sessions[0].done === true,
  );

  const empty = M.exerciseStats(ex, [], settings, anchor, new Set([anchor]));
  ok(
    "a lift with nothing logged can still be called done",
    empty.doneToday === true,
  );
}

/* -------------------------------------------- 11. the hypertrophy dose */

// --- tonnage survives as a readout, and is not asked to judge anything ---
{
  ...
  close(
    "a lift with nothing to load is counted in reps",
    M.work(0, 12, 3, "bodyweight"),
    36,
  );
  ok(
    "and never mixes the two — press-ups are not zero kilos",
    M.work(0, 12, 3, "bodyweight") === 36 && M.work(0, 12, 3, "weight") === 0,
  );

  const day = [
    { weight: 100, reps: 5, sets: 3 },
    { weight: 60, reps: 10, sets: 2 },
  ];
  close(
    "a session is every block it held",
    M.sessionWork(day, "weight"),
    1500 + 1200,
  );
  close("and nothing is not a crash", M.sessionWork([], "weight"), 0);

  // Why it cannot be the hypertrophy measure: these are the same number.
  close(
    "a heavy triple and five sets of eight move the same tonnage",
    M.work(926, 3, 1, "weight"),
    M.work(71, 8, 5, "weight"),
    70,
  );
  ok(
    "but they are nothing like the same stimulus",
    M.hardSets([{ weight: 926, reps: 3, sets: 1 }]) === 0 &&
      M.hardSets([{ weight: 71, reps: 8, sets: 5 }]) === 5,
    `${M.hardSets([{ weight: 926, reps: 3, sets: 1 }])} vs ${M.hardSets([{ weight: 71, reps: 8, sets: 5 }])}`,
  );
}

// --- what a rep count is worth as a hard set ---
{
  const c = (r) => M.repCredit(r);
  ok(
    "a heavy single is not hypertrophy work at all",
    c(1) === 0 && c(2) === 0 && c(3) === 0,
  );
  ok(
    "it ramps in below the range rather than switching on",
    c(4) > 0 && c(5) > c(4) && c(6) === 1,
    `${c(4)}, ${c(5)}, ${c(6)}`,
  );
  ok(
    "the whole range counts whole",
    [6, 8, 10, 12, 14, 16, 20].every((r) => c(r) === 1),
  );
  ok(
    "and it tapers out above it rather than stopping dead",
    c(25) > 0 && c(25) < 1 && c(30) === 0,
    `${c(25)}, ${c(30)}`,
  );
  ok("junk reps are worth nothing", c(0) === 0 && c(-5) === 0 && c(NaN) === 0);
}

// --- and what proximity to failure is worth ---
{
  const f = (v) => M.failureCredit(v);
  ok(
    "nought to two in reserve is the productive zone",
    f(0) === 1 && f(1) === 1 && f(2) === 1,
  );
  ok("past that it falls away", f(3) < 1 && f(4) < f(3) && f(6) < f(4));
  ok("but never to nothing — a set left well short is still a set", f(10) > 0);
  ok(
    "an unrecorded RIR is assumed to have been a hard set",
    f(null) === 1 && f(undefined) === 1 && f("") === 1,
  );
  ok(
    "so a log kept without RIR does not read as no training at all",
    M.hardSets([{ weight: 60, reps: 10, sets: 3 }]) === 3,
  );
}

// --- hard sets across a log, using the per-set record where there is one ---
{
  close(
    "three sets in range at two in reserve are three hard sets",
    M.hardSets([{ weight: 60, reps: 10, sets: 3, rir: 2 }]),
    3,
  );
  ok(
    "the same three taken well short are worth less",
    M.hardSets([{ weight: 60, reps: 10, sets: 3, rir: 5 }]) < 3,
  );

  // A merged block keeps the LOWEST rir of the sets counted onto it, so using
  // it for all of them would credit five sets as though all had finished hard.
  const withLog = [
    {
      weight: 60,
      reps: 10,
      sets: 3,
      rir: 0,
      log: [{ rir: 4 }, { rir: 2 }, { rir: 0 }],
    },
  ];
  ok(
    "per-set RIR is used where the log has it, rather than the block minimum",
    M.hardSets(withLog) < 3 && M.hardSets(withLog) > 2,
    String(M.hardSets(withLog)),
  );
  ...
  const noRir = [
    {
      ...
      sets: 3,
      rir: 1,
      log: [{ rir: null }, { rir: null }, { rir: null }],
    },
  ];
  close(
    "a log with no RIR in it falls back to the block",
    M.hardSets(noRir),
    3,
  );
  close("nothing logged is no dose", M.hardSets([]), 0);
}

// --- the weekly budget, counted properly ---
{
  const anchor = "2026-01-01";
  const ex = {
    id: "x",
  ...
  const at = (entries) => M.exerciseStats(ex, entries, settings, anchor);
  ...
  ok(
    "twelve sets of twelve is twelve hard sets",
    Math.abs(twelves.hardSets7 - 12) < 1e-9,
  );
  ok("and reads as on target", twelves.hardSetStatus === "on target");
  ...
  ok(
    "twelve sets of three is still twelve sets by the plain count",
    triples.sets7 === 12,
  );
  ok("and on target by it", triples.setStatus === "on target");
  ok("but no hypertrophy dose whatsoever", triples.hardSets7 === 0);
  ok("so the honest status says under", triples.hardSetStatus === "under");

  const none = at([]);
  ok(
    "a lift with nothing logged has no dose and no verdict",
    none.hardSets7 === 0 && none.hardSetStatus === null,
  );
}

// --- both picks come off the same target now, and differ only in reps ---
{
  const anchor = "2026-01-01";
  const ex = {
    id: "x",
  ...
  ok(
    "the strength pick is in the strength range",
    M.inZone(plan.picks.strength.reps, M.ZONES.strength),
  );
  ok(
    "the size pick is in the size range",
    M.inZone(plan.picks.hypertrophy.reps, M.ZONES.hypertrophy),
  );
  ok(
    "both clear the one target",
    plan.picks.strength.score >= plan.target - 1e-9 &&
      plan.picks.hypertrophy.score >= plan.target - 1e-9,
  );
  ok(
    "the size pick is the lighter, longer set",
    plan.picks.hypertrophy.reps > plan.picks.strength.reps &&
      plan.picks.hypertrophy.weight < plan.picks.strength.weight,
  );
  ok(
    "the two are comparable — same sets",
    plan.picks.hypertrophy.sets === plan.picks.strength.sets,
  );
  ok(
    "neither sits on the boundary the other shares",
    plan.picks.strength.reps < 6 && plan.picks.hypertrophy.reps > 6,
    `${plan.picks.strength.reps} and ${plan.picks.hypertrophy.reps}`,
  );

  ok(
    "the size pick is worth a full dose",
    plan.picks.hypertrophy.hardSets === plan.sets,
  );
  ok(
    "and the strength pick honestly is not",
    plan.picks.strength.hardSets < plan.sets,
  );

  // Your own rep count stands when it is already in range.
  ok(
    "somebody doing triples is not told five is the strength scheme",
    M.planFor(st, settings, { reps: 3, sets: 3 }).picks.strength.reps === 3,
  );
  ok(
    "and somebody doing sixteens keeps them on the size side",
    M.planFor(st, settings, { reps: 16, sets: 3 }).picks.hypertrophy.reps ===
      16,
  );

  // There is one grid again, and the zone does not change the sum.
  ok(
    "asking for size does not change the weight at the same reps",
    M.planFor(st, settings, { reps: 12, sets: 3 }).weight ===
      M.planFor(st, settings, { reps: 12, sets: 3, zone: "hypertrophy" })
        .weight,
  );
  ok(
    "the zone only says which pick is current",
    M.planFor(st, settings, { zone: "hypertrophy" }).zone === "hypertrophy",
  );
}

// --- the grid reaches the size range now ---
{
  ok(
    "the axis runs past what the sheet had",
    Math.max(...M.REP_SCHEMES) === 20,
    M.REP_SCHEMES.join(),
  );
  ok(
    "every rep scheme in the size range is on it",
    [6, 8, 10, 12, 14, 16, 20].every((r) => M.REP_SCHEMES.includes(r)),
  );
  ok(
    "the zones meet at six and nowhere else",
    M.zonesFor(6).length === 2 &&
      M.zonesFor(5).length === 1 &&
      M.zonesFor(8).length === 1,
  );
  ok(
    "twenty reps is still size, twenty-five is off the grid",
    M.inZone(20, M.ZONES.hypertrophy) && !M.inZone(25, M.ZONES.hypertrophy),
  );
}
...
console.log(
  "The app agrees with the spreadsheet on every value it publishes.\n",
);
